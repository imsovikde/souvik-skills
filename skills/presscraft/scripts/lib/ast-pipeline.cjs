...
`;
  ...
  function renderDocument(markdownText) {
    ...
    // Transform blockquotes with [!NOTE], [!TIP], [!IMPORTANT], [!WARNING], [!CAUTION]
    const calloutRegex = /<blockquote>\s*<p>\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\](?:\s*<br\s*\/?>)?([\s\S]*?)<\/blockquote>/gi;
